...
export function Connections() {
  ...
  const resetAddForm = () => {
    ...
    setServerName('');
    setApiKey('');
    setDispatcharr(emptyDispatcharrForm());
    setConnectError(null);
  ...
  const handleAddServer = async () => {
    const hasAuth =
      serverType === 'dispatcharr'
        ...
        : Boolean(apiKey);
    if (!serverUrl || !serverName || !hasAuth) {
    ...
    try {
      if (serverType === 'dispatcharr') {
        await api.servers.create({
          ...
          name: serverName,
          url: serverUrl,
          ignoreAnonymousStreams: dispatcharr.ignoreAnonymousStreams,
          ...(dispatcharr.mode === 'token'
  ...
  return (
    ...
    >
      ...
      <AddServerDialog
        ...
        onDispatcharrChange={setDispatcharr}

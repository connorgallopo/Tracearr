...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_sessions_media_type_ended
  ON sessions(media_type, ended_at DESC);

-- Composite index for server + media type queries
-- Covers: per-server stats filtered by media type
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_sessions_server_media_type
  ON sessions(server_id, media_type, ended_at DESC);
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_sessions_music_artist
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_sessions_live_channel
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_sessions_user_media_type
  ON sessions(user_id, media_type, ended_at DESC);
